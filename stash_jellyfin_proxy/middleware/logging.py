...
from stash_jellyfin_proxy.stash.scene import get_scene_info
...
class RequestLoggingMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        if is_stream:
            await self._track_stream_event(path, headers, client_host, ms=0)
    ...
    async def _track_stream_event(self, path: str, headers: dict, client_host: str, ms: int) -> None:
        ...
        if not cached_file_size:
            cached_file_size = (await get_scene_info(scene_id)).get("file_size", 0)
        ...
        if stream_info is None:
            ...
            if stopped_at and (now - stopped_at) < _streams.RECENTLY_STOPPED_GRACE:
                ...
                scene_info = await get_scene_info(scene_id)
